...
static const char* inet_ntopA(int af, const void *cp, char *buf, socklen_t len)
{
	...
	if (af == AF_INET)
	{
		LPSOCKADDR_IN ip4 = (void *)&any;
		CopyMemory(&ip4->sin_addr, cp, sizeof(IN_ADDR));
		AFlen = sizeof(SOCKADDR_IN);
	}
	else if (af == AF_INET6)
	{
		LPSOCKADDR_IN6 ip6 = (void *)&any;
		CopyMemory(&ip6->sin6_addr, cp, sizeof(IN6_ADDR));
		AFlen = sizeof(SOCKADDR_IN6);
	}

	if (WSAAddressToStringA((LPSOCKADDR)&any, (DWORD)AFlen, NULL, buf, &Dlen) == SOCKET_ERROR)
	{
		printf("inet_ntopA error: %d\n", WSAGetLastError());
		return NULL;
	}
	return buf;
}
