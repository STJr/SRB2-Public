...
//-----------------------------------------------------------------------------
/// \file
/// \brief midi-2-stream conversion
///	adapted from DirectX6 sample code
///
///	a lot of code is duplicate : the standalone version
...
} ATTRPACK MIDIFILEHDR, *LPMIDIFILEHDR;
...
//
typedef struct STREAMBUF *LPSTREAMBUF;
typedef struct STREAMBUF
{
	...
	LONG               iBytesLeft;             // BYTEs left in buffer
	LONG               iBytesLeftUncompressed; // BYTEs left when uncompressed
	LPSTREAMBUF        pNext;                  // Next buffer
} STREAMBUF;
...
typedef struct
{
	...
	LPSTREAMBUF        pFirst;                 // First stream buffer
	LPSTREAMBUF        pLast;                  // Last (current) stream buffer
} OUTSTREAMSTATE;
...
#ifndef LEGACY
static VOID   I_OutputMsg(LPSTR fmt, ...);
static VOID   I_Error(LPSTR error, ...);
