...
//
static inline void HWR_SubsecPoly(int num, poly_t *poly)
{
	short count;
	subsector_t *sub;
	seg_t *lseg;
...
static void SearchSegInBSP(int bspnum,polyvertex_t *p,poly_t *poly)
{
	poly_t  *q;
	int     j,k;
	...
	for (l = 0; l < addsubsector; l++)
	{
		...
		if (p)
			for (i = 0; i < p->numpts; i++)
				SearchSegInBSP((int)numnodes-1, &p->pts[i], p);
	}
