...
static INT32 SCY(INT32 y)
{
	...
	y = FixedDiv(y,vid.fdupy); // scale to resolution
	if (splitscreen)
	{
		y >>= 1;
	...
	patch_t **numpat)
{
	...
	if (!num)
	{
		V_DrawScaledPatch(x - FixedMul(w, vid.dupx), y, V_NOSCALESTART|V_TRANSLUCENT, numpat[0]);
		return;
	}
...
static void ST_drawLevelTitle(void)
{
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(200, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(188, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(0), FixedMul(176, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(164, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(152, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(140, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(128, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(80, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(56, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(32, vid.fdupy), V_NOSCALESTART, ttlnum);
...
//		V_DrawCenteredString(subttlxpos, 152+48, 0, subttl);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(8, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(0, vid.fdupy), V_NOSCALESTART, ttlnum);
	...
	{
		if (!nonumber)
			V_DrawScaledPatch(SCX(ttlnumxpos), FixedMul(104, vid.fdupy), V_NOSCALESTART, ttlnum);
...
static void ST_drawNiGHTSHUD(void)
{
	...
	V_DrawScaledPatch(SCX(24), SCY(8) + FixedMul(8, vid.fdupy), V_NOSCALESTART|V_TRANSLUCENT, nhud[(leveltime/2)%12]);
	...
	if (stplyr->capsule && !cv_objectplace.value)
	{
		...
		V_DrawScaledPatch(SCX(74), SCY(8) + FixedMul(4, vid.fdupy), V_NOSCALESTART|V_TRANSLUCENT,
			minicaps);
		...
		V_DrawScaledPatch(SCX(40), SCY(8) + FixedMul(5, vid.fdupy), V_NOSCALESTART|V_TRANSLUCENT, narrow[(leveltime/2)%8]);
	}
	else
		V_DrawScaledPatch(SCX(40), SCY(8) + FixedMul(5, vid.fdupy), V_NOSCALESTART|V_TRANSLUCENT, narrow[8]);

	ST_DrawOverlayNum(SCX(68), SCY(8) + FixedMul(11, vid.fdupy), stplyr->health > 0 ? stplyr->health - 1 : 0, tallnum);
...
static void ST_drawSpecialStageHUD(void)
{
	...
	{
		V_DrawScaledPatch(hudinfo[HUD_GETRINGS].x, FixedInt(FixedDiv(SCY(hudinfo[HUD_GETRINGS].y)*FRACUNIT, vid.fdupy)), V_TRANSLUCENT, getall);
